...
#pragma omp parallel for schedule(dynamic, 32) num_threads(16)
...
bool findPathWithExhaustiveSearch(ProblemData &problemData, int timestep,
    ...
    bool result = false;
#pragma omp parallel for schedule(dynamic, 32) reduction(+:numPossiblePositions) num_threads(16)
    for (int x = 0; x < MAP_SIZE; ++x) {
        if(result)
            continue;
        for (int y = 0; y < MAP_SIZE; ++y) {
...
int main(int argc, char *argv[]) {
    ...
    //TODO Enabling a feature in the runtime library
    //omp_set_nested(2);

    // Note that on the submission server, we are solving "numProblems" problems
    // TODO Parallelizing the main routine across the different problems
//#pragma omp parallel for ordered num_threads(6)
    for (int problem = 0; problem < numProblems; ++problem) {
        ...
        // TODO Ensuring that our "not thread-safe" functions are not executed in parallel
//#pragma omp critical 
        ...
        // TODO Outputting values in the correct order
//#pragma omp ordered
